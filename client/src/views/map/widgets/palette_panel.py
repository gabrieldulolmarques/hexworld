...
from models.map.tool_ids import TOOL_DESCRIPTION, TOOL_SELECT, TOOL_TERRAIN
...
class PalettePanel(MapPanel):
    ...
    _GRID_GAP = 10
    _BTN_W = (
        SIDE_PANEL_W - 2 * _PANEL_PAD - _SCROLL_GUTTER - (_COLS - 1) * _GRID_GAP
    ) // _COLS
    ...
    def __init__(self) -> None:
        ...
        self._terrain_stem: str | None = None
        ...
        for r in range(rows):
            ...
            for c in range(self._BIOME_COLS):
                ...
                label = _BIOME_LABELS.get(biome, biome.replace("_", " ").title())
                tab = QToolButton()
        ...
        self._terrain_area = QWidget()
        self._terrain_area.setObjectName("mapTerrainArea")
        self._terrain_area.setSizePolicy(
            QSizePolicy.Policy.Preferred,
            QSizePolicy.Policy.Expanding,
        )
        styled(self._terrain_area)
        terrain_lay = QVBoxLayout(self._terrain_area)
        terrain_lay.setContentsMargins(0, 0, 0, 0)
        terrain_lay.setSpacing(8)
        terrain_lay.addWidget(self._tab_strip)
        terrain_lay.addWidget(self._scroll, 1)

        self._heading = QLabel("TERRAIN")
        ...
        layout.addWidget(self._terrain_area, 1)
    ...
    def _on_biome_clicked(self, index: int) -> None:
        ...
        if biome == _EMPTY_CAT:
            entries = [
                (
                    info,
                    _BIOME_LABELS.get(info.biome, info.biome.replace("_", " ").title()),
                )
    ...
    def _apply_tool(self, tool_id: str, *, emit: bool = True) -> None:
        ...
        self._terrain_area.setVisible(tool_id == TOOL_TERRAIN)
