...
from views.map.widgets.icons import tinted_pixmap
...
_TOOLS: tuple[tuple[str, str, str], ...] = (
    ...
    (TOOL_TERRAIN, "tool-terrain.svg", "Terrain"),
    (TOOL_PATH, "tool-path.svg", "Path"),
...
class ToolStrip(MapPanel):
    ...
    def _update_button_icon(self, btn: QToolButton) -> None:
        ...
        key = (icon_file, color, 20)
        icon = self._icon_cache.get(key)
        if icon is None:
            icon = QIcon(tinted_pixmap(icon_file, color, 20))
    ...
    def set_active_tool(self, tool_id: str) -> None:
        index = self._tool_ids.index(tool_id) if tool_id in self._tool_ids else -1
        if index >= 0:
