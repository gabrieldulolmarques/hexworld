...
from views.lobby.map_card import MapCard
from views.shared.widgets import HexLogo, StatusMixin
...
class MapGrid(StatusMixin, QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        grid_width = (
            _GRID_COLS * CARD_WIDTH + (_GRID_COLS - 1) * _GRID_GAP + _GRID_MARGIN_H
        )
    ...
    def set_maps(self, maps: list[dict]) -> None:
        ...
        for map_id in list(self._cards):
            if map_id not in new_ids:
                self._cards.pop(map_id).deleteLater()
    ...
    def remove_map(self, map_id: str) -> None:
        ...
        card = self._cards.pop(map_id, None)
        if card:
            card.deleteLater()
        self._rebuild_cards()
    ...
    def _invalidate_card(self, map_id: str) -> None:
        card = self._cards.pop(map_id, None)
        if card:
            card.setParent(None)
            card.deleteLater()
